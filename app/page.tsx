...
export default function Home() {
  ...
  useEffect(() => {
    if (isPending) toast.loading('Check your wallet...', { id: 'mint' });
    if (isConfirming) toast.loading('Confirming on Neura...', { id: 'mint' });
    if (isSuccess) toast.success('SoulCard Minted! 🎉', { id: 'mint' });
    if (error) {
      console.error(error);
      toast.error('RPC Error: Please RESET MetaMask Account.', { id: 'mint' });
    }
  ...
  const handleMint = () => {
    ...
    writeContract({
      ...
      // Aumentamos o gas para 3 milhões para forçar o RPC a aceitar
      gas: 3000000n, 
    });
  };
  ...
  return (
    <main className="min-h-screen flex flex-col items-center justify-center p-6 bg-slate-50">
      ...
      <div className="w-full max-w-6xl grid grid-cols-1 lg:grid-cols-2 gap-8 items-stretch">
          ...
          <div className="space-y-6">
            ...
            {isConnected && (
              <div className="space-y-4 text-left">
                ...
                <textarea value={bio} placeholder="Your Bio..." className="w-full p-4 bg-slate-50 border rounded-2xl min-h-[100px]" onChange={(e) => setBio(e.target.value)} />
              </div>
            )}
          </div>

          {isConnected && (
            <button onClick={handleMint} disabled={isPending || isConfirming} className="w-full py-5 rounded-2xl font-black text-white bg-blue-600 hover:bg-blue-700 uppercase tracking-widest transition-all shadow-lg shadow-blue-200">
        ...
        <div className="hidden lg:flex items-center justify-center bg-slate-200/40 rounded-[2.5rem] border-2 border-dashed border-slate-300 min-h-[500px]">
          <div className="relative w-full max-w-sm aspect-[3/4] rounded-[2.5rem] overflow-hidden shadow-2xl transition-all duration-500">
            ...
            <div className="relative z-10 p-8 flex flex-col justify-between h-full text-white text-left">
              ...
              <div className="space-y-4 text-left">
                <div className="w-16 h-16 bg-white/20 rounded-2xl backdrop-blur-md flex items-center justify-center text-3xl font-black italic">
              ...
              <div className="flex justify-between items-end border-t border-white/20 pt-4 text-[10px] font-black uppercase">
                <span>NEURA TESTNET</span>
                <span className="bg-white text-blue-600 px-3 py-1 rounded-full">Verified</span>
              </div>
            </div>
          </div>
        </div>
